---
title: "Group Presentation for RS"
...
- Maximum 1-Day Precipitation (MIDP)

4.Pedological Data: Harmonized World Soil Database (downscaled to 30m)
- Soil Texture (ST): 1-8 (sand → clay)

---
# Machine Learning Modeling--Modeling Steps
Train models to assess indicator importance and generate flood risk maps.

1.Data Integration
- Align indicator data with historical flood labels
- Divide training set (70%) and test set (30%) using watershed spatial stratified sampling

2.Model Selection
- Primary Model: Random Forest (RF)
- Comparative Model: XGBoost (to validate robustness)
...
- AUC-ROC (>0.9 considered excellent)
- SHAP analysis: Quantify indicator contributions
---
# Definition and Calculation of 11 Flood Risk Indicators

1.Topographic Indicators (Derived from SRTM DEM)
- Elevation (El): Low-lying areas show significantly higher risk
- Slope (Sl): steep slopes accelerate runoff
- Slope Aspect (SA): North-facing slopes more prone to water accumulation
- River Distance (RD): Extracting the Euclidean distance of the river
- Watercourse Density (WRD): Total river length / Watershed area
- TWI (Topographic Wetness Index): ln(Flow accumulation / tan(Slope))

2.Spectral Indicators (Landsat 8 Calculations)
- NDVI: (Band5 - Band4) / (Band5 + Band4)
- NDWI: (Band3 - Band5) / (Band3 + Band5)
- Land Use (LU)：
